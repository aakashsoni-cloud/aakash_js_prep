...
// Brute TC : O(N^2) SC : O(unique numbers) using set 


// Better TC : O(N) + O(N) ~~ O(2N)  SC: O(unique numbers)
...
// Optimized TC: O(N) SC: O(unique numbers) making inside while to if reduce till the size of bucket become 2
function fruitInBasket(fruits) {
  let l = 0;
  let r = 0;
